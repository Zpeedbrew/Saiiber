...
LOG_DEBUG("blSrc:X(%f),Y(%f),Z(%f),W(%f)\n",blSrc.x,blSrc.y,blSrc.z,blSrc.w);
LOG_DEBUG("trSrc: X(%f),Y(%f),Z(%f),W(%f)\n",trSrc.x,trSrc.y,trSrc.z,trSrc.w);
*/
...
inView=left<SCREEN_WIDTH&&right>0&&bottom<SCREEN_HEIGHT&&top>0;
//Round to nearest even number
ppSource=glm::u16vec4(roundf(left/2.0f)*2,SCREEN_HEIGHT-(roundf(top/2.0f)*2),roundf(abs(right-left)/2)*2,roundf(abs(bottom-top)/2)*2);
}
void Saber::render() {
